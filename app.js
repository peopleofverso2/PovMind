...
const state = {
    ...
    enrichmentRuns: loadEnrichmentRuns(),
    cognitiveCycles: loadCognitiveCycles(),
};
function uid() {
...
function cleanVaultRegistry(value) {
    if (!value || typeof value !== "object" || !Array.isArray(value.vaults))
        return null;
    const seen = new Set();
...
function cleanEncryptionState(value) {
...
async function deriveVaultCryptoKey(passphrase) {
    if (!crypto.subtle)
        throw new Error("Web Crypto indisponible");
    const salt = state.security.encryption.salt;
    if (!salt)
        throw new Error("Sel de chiffrement manquant");
...
async function encryptJsonPayload(payload, key) {
...
async function decryptJsonPayload(sealed, key) {
...
async function persistEncryptedNotes(showSaved = true) {
    ...
    if (showSaved)
        els.savedStatus.textContent = "Chiffré";
    touchActiveVault();
}
async function unlockEncryptedVault(passphrase) {
    ...
    state.enrichmentRuns = Array.isArray(parsed.enrichmentRuns)
        ? parsed.enrichmentRuns.map(cleanEnrichmentRun).filter(Boolean).slice(0, MAX_ENRICHMENT_RUNS)
        : [];
    state.cognitiveCycles = Array.isArray(parsed.cognitiveCycles)
        ? parsed.cognitiveCycles.map(cleanCognitiveCycle).filter(Boolean).slice(0, MAX_COGNITIVE_CYCLES)
        : [];
    if (!state.starredIds.size && state.activeId)
        state.starredIds.add(state.activeId);
    persistStarredIds();
}
...
function persistEnrichmentRuns() {
    state.enrichmentRuns = state.enrichmentRuns.map(cleanEnrichmentRun).filter(Boolean).slice(0, MAX_ENRICHMENT_RUNS);
    if (vaultEncrypted()) {
        localStorage.removeItem(vaultStorageKey("enrichment-runs"));
        void persistEncryptedNotes(false).catch((error) => {
            console.error(error);
...
function persistCognitiveCycles() {
    state.cognitiveCycles = state.cognitiveCycles.map(cleanCognitiveCycle).filter(Boolean).slice(0, MAX_COGNITIVE_CYCLES);
    if (vaultEncrypted()) {
        localStorage.removeItem(vaultStorageKey("cognitive-cycles"));
        void persistEncryptedNotes(false).catch((error) => {
            console.error(error);
...
function describePovChatSyncState(sync) {
    ...
    const label = latestIsPull
        ...
        : pushedAt
            ...
            : "Sync Cloud SQL";
    const details = [];
    if (Number.isFinite(Number(sync.noteCount)))
        details.push(`${Number(sync.noteCount)} notes publiées`);
    if (Number.isFinite(Number(sync.remoteNoteCount)))
        details.push(`${Number(sync.remoteNoteCount)} notes serveur`);
    if (Number(sync.chatNoteCount) > 0)
        details.push(`${Number(sync.chatNoteCount)} notes chat`);
    if (sync.remoteLastSyncedAt)
        details.push(`distant ${formatDate(sync.remoteLastSyncedAt)}`);
    details.push(auto);
...
function persistLayoutSettings() {
    localStorage.setItem(vaultStorageKey("layout"), JSON.stringify({
        version: 1,
...
function clampEditorPaneWidth(width) {
    ...
    if (available < minPane * 2)
        return Math.max(180, available / 2);
...
function applyLayoutSettings(preferredKind = null) {
    ...
    root.style.setProperty("--editor-pane-width", state.layout.editorPaneWidth ? `${state.layout.editorPaneWidth}px` : "1fr");
...
function ensureDocumentationVault(options = {}) {
    ...
    if (created.length || updated.length || select)
        renderAll();
    if (!silent)
        toast(created.length || updated.length ? "Vault documentation PovMind mis à jour." : "Vault documentation PovMind ouvert.");
}
function loadStore() {
    if (vaultEncrypted()) {
        state.notes = [];
    ...
    try {
        ...
        if (parsed.learningMemory)
            state.learningMemory = cleanLearningMemory(parsed.learningMemory);
        if (Array.isArray(parsed.enrichmentRuns)) {
            state.enrichmentRuns = parsed.enrichmentRuns.map(cleanEnrichmentRun).filter(Boolean).slice(0, MAX_ENRICHMENT_RUNS);
        }
        if (Array.isArray(parsed.cognitiveCycles)) {
            state.cognitiveCycles = parsed.cognitiveCycles.map(cleanCognitiveCycle).filter(Boolean).slice(0, MAX_COGNITIVE_CYCLES);
        }
...
function persistNow(showSaved = true) {
    ...
    localStorage.setItem(vaultStorageKey("notes"), JSON.stringify(notesPlainPayload()));
    if (showSaved) {
...
function uniqueTitle(baseTitle) {
    ...
    if (!findNoteByTitle(base))
        return base;
    let index = 2;
    while (findNoteByTitle(`${base} ${index}`))
        index += 1;
    return `${base} ${index}`;
}
...
function createNote(title = "Nouvelle note", body = "", options = {}) {
    if (!requireVaultUnlocked("créer une note"))
        return null;
    const createdAt = nowIso();
    const note = {
        id: uid(),
...
function deleteActiveNote() {
    if (!requireVaultUnlocked("supprimer une note"))
        return;
    ...
    if (!state.activeId)
        createNote("Accueil", "# Accueil\n\nTon carnet est prêt.");
    queueSave();
    renderAll();
    toast("Note supprimée.");
}
function selectNote(id) {
    if (!requireVaultUnlocked("ouvrir une note"))
        return;
...
function toggleActiveNoteMemoryType(typeId) {
    if (!requireVaultUnlocked("classifier cette note"))
        return;
    const note = activeNote();
    ...
    if (next.length)
        note.memoryTypes = next;
    else
        delete note.memoryTypes;
...
function allTags() {
    ...
    for (const note of state.notes) {
        for (const tag of tagsForNote(note))
...
function makeProposal(type, title, detail, options = {}) {
...
function buildDeterministicEnrichmentProposals() {
    ...
    if (untypedNotes.length) {
        proposals.push(makeProposal("assign_memory", "Classifier les mémoires", `${untypedNotes.length} note(s) peuvent recevoir une couche mémoire explicite.`, {
            confidence: 0.78,
            risk: "low",
    ...
    if (active && !findNoteByTitle("Contexte court - Session")) {
        proposals.push(makeProposal("create_context", "Créer la mémoire immédiate", "Synthétiser la note active en contexte court pour réduire la charge mentale.", {
            targetId: active.id,
    ...
    if (snapshotNeedsRefresh()) {
        proposals.push(makeProposal("create_snapshot", "Figer un snapshot du vault", "Le vault ou son repo lié a changé depuis le dernier hash stable.", {
            confidence: 0.82,
            risk: "low",
    ...
    if (repoIsLinked()) {
        ...
        if (!findNoteByTitle(repoNoteTitle)) {
            proposals.push(makeProposal("repo_note", "Créer la note du repo", "Ancrer le vault au repo exécutable avec commit, branche et fichiers indexés.", {
    ...
    if (active && textIncludesAny(`${active.title}\n${active.body}`, ["paradoxe", "contradiction", "tension", "inverse", "mais aussi"])) {
        ...
        if (!findNoteByTitle(tensionTitle)) {
            proposals.push(makeProposal("create_tension", "Isoler une tension paradoxale", "La note active contient un conflit fertile qui mérite une fiche de clarification.", {
                targetId: active.id,
    ...
    if (hasDreamVocabulary && !findNoteByTitle("Cycle rêve - Journal")) {
        proposals.push(makeProposal("dream_cycle", "Préparer un cycle rêve", "Créer une note de consolidation nocturne : compression, mutation, oubli doux et scénarios.", {
    ...
    if (proposals.length && !hasTodaysEnrichmentReport()) {
        proposals.push(makeProposal("create_report", "Documenter l'analyse", "Créer une note de rapport pour garder la boucle auto-apprenante auditée.", {
            confidence: 0.7,
            risk: "low",
...
function runDeterministicEnrichment(showToast = true) {
    if (!requireVaultUnlocked("analyser le vault"))
        return null;
    persistNow(false);
    const stats = graphStats();
    ...
    renderLearningPanel();
    if (showToast)
        toast(proposals.length ? `${proposals.length} suggestion(s) générée(s).` : "Analyse terminée : rien d'urgent.");
    return run;
}
...
function createEnrichmentReport(run = latestEnrichmentRun()) {
    if (!requireVaultUnlocked("créer le rapport d'enrichissement"))
        return null;
...
function recordLearningFeedback(kind, proposal) {
    ...
    if (kind === "accepted")
        memory.acceptedPatterns = [entry, ...(memory.acceptedPatterns || [])].slice(0, 80);
    if (kind === "rejected")
        memory.rejectedPatterns = [entry, ...(memory.rejectedPatterns || [])].slice(0, 80);
...
async function applyEnrichmentProposal(proposalId) {
    if (!requireVaultUnlocked("appliquer cette suggestion"))
        return;
    const found = findProposalWithRun(proposalId);
    if (!found || found.proposal.status !== "pending")
        return;
    ...
    if (proposal.type === "assign_memory") {
        ...
        for (const item of proposal.evidence || []) {
            ...
            if (!note || cleanMemoryTypes(note.memoryTypes).length)
                continue;
...
function rejectEnrichmentProposal(proposalId) {
    if (!requireVaultUnlocked("ignorer cette suggestion"))
        return;
    const found = findProposalWithRun(proposalId);
    if (!found || found.proposal.status !== "pending")
        return;
...
function transformEnrichmentProposalToAction(proposalId) {
    if (!requireVaultUnlocked("transformer cette suggestion en action"))
        return;
    const found = findProposalWithRun(proposalId);
    if (!found || found.proposal.status !== "pending")
        return;
...
function calculateLearningScore() {
    ...
    const ratio = clamp(memoryCoverage * 0.22 +
        linkDensity * 0.18 +
...
async function runCognitiveDayCycle() {
    if (!requireVaultUnlocked("lancer le cycle jour"))
        return null;
...
async function runCognitiveNightCycle() {
    if (!requireVaultUnlocked("lancer le cycle nuit"))
        return null;
    const run = runDeterministicEnrichment(false);
...
async function exportCognitiveCronBundle() {
    if (!requireVaultUnlocked("exporter le cron cognitif"))
        return null;
...
function persistGraphPositions() {
    localStorage.setItem(vaultStorageKey("graph-layout"), JSON.stringify({
        version: 1,
        updatedAt: nowIso(),
...
function forceDirectedGraphPositions(nodes, edges, settings = GRAPH_FORCE_DEFAULTS) {
    const width = 540;
    const height = 380;
    const cx = width / 2;
    const cy = height / 2;
...
function buildDefaultGraphPositions(nodes, edges = []) {
    return forceDirectedGraphPositions(nodes, edges);
}
function resolveGraphPositions(nodes, edges = []) {
    ...
    if (newlyPlaced > 0) {
        ...
        catch (_) { /* ignore localStorage quota */ }
    }
...
function handleGraphPointerDown(event) {
    ...
    if (!node || !els.graph.contains(node))
        return;
...
async function generateAssistantToken(showToast = true) {
    try {
        ...
        if (showToast)
            toast("Token crypto généré. Copie-le maintenant : le secret n'est pas stocké.");
        return token;
    }
    catch (error) {
        console.error(error);
...
async function ensureAssistantTokenForExport() {
    if (state.security.tokenHash)
        return state.assistantToken;
...
async function readStoredZipEntries(file) {
    ...
    while (offset + 30 <= bytes.length) {
        ...
        if (method !== 0)
            throw new Error("ZIP compressé non supporté par cet import léger");
        if (flags & 0x0008)
            throw new Error("ZIP avec data descriptor non supporté");
        if (compressedSize !== uncompressedSize)
            throw new Error("Entrée ZIP incohérente");
        const name = decoder.decode(bytes.slice(offset + 30, offset + 30 + fileNameLength)).replaceAll("\\", "/");
        if (name && !name.endsWith("/"))
            entries[name] = decoder.decode(bytes.slice(dataOffset, dataEnd));
        offset = dataEnd;
    }
    return entries;
}
function uniqueWikiTargets(markdown) {
    ...
    for (const title of extractWikiLinks(markdown)) {
        ...
        if (!targets.has(normalized))
            targets.set(normalized, { normalized, title });
    }
...
async function createVaultSnapshot(options = {}) {
    if (!requireVaultUnlocked("créer un snapshot"))
        return null;
    persistNow(false);
    const createdAt = nowIso();
    ...
    renderSnapshotsPanel();
    if (!options.silent)
        toast(`Snapshot créé : ${hash.slice(0, 12)}…`);
    return snapshot;
}
async function exportSnapshot(snapshotId = null) {
    ...
    if (!snapshot)
        snapshot = await createVaultSnapshot({ silent: true });
    if (!snapshot)
        return;
...
function exportCodexKnowledgeBase() {
    if (!requireVaultUnlocked("exporter la base Codex"))
        return;
    persistNow(false);
...
async function exportMcpBundle() {
    if (!requireVaultUnlocked("exporter le bundle MCP"))
        return;
    persistNow(false);
...
async function exportGithubContextBundle(showToast = true) {
    if (!requireVaultUnlocked("exporter le contexte GitHub"))
        return null;
    ...
    if (showToast)
        toast(`Contexte GitHub exporté : ${context.files.length} note(s).`);
    return { context, files, snapshot };
}
...
async function pushGithubContext() {
    if (!requireVaultUnlocked("pousser le contexte GitHub"))
        return;
    syncGithubSettingsFromInputs();
...
async function pullGithubContext() {
    if (!requireVaultUnlocked("tirer le contexte GitHub"))
        return;
    syncGithubSettingsFromInputs();
    if (!state.githubSync.repoFullName) {
        toast("Renseigne un repo GitHub owner/repo.");
...
function exportVault() {
    if (!requireVaultUnlocked("exporter le vault"))
        return;
    persistNow(false);
    const payload = {
        version: 1,
...
function exportMarkdown() {
    if (!requireVaultUnlocked("exporter la note"))
        return;
    const note = activeNote();
    if (!note)
        return;
...
async function importRepoManifest(file) {
    ...
    try {
        ...
        if (!manifest.name && !manifest.root)
            throw new Error("Nom de repo manquant");
        state.repo = manifest;
        persistRepoState();
        renderRepoPanel();
...
function ensureCodeRepoNote() {
    if (!requireVaultUnlocked("créer la note code repo"))
        return null;
    if (!repoIsLinked()) {
...
function buildDevVaultImportPayload(repoInput, syncInput = null) {
    ...
    const indexNote = addNote(`${repoName} - Dev Index`, "Dev Context", "");
...
async function scanGithubRepoToVault(mode = "new") {
    if (!requireVaultUnlocked("scanner un repo GitHub"))
        return;
    syncGithubSettingsFromInputs();
    const sync = cleanGithubSyncState(state.githubSync);
    ...
    try {
        ...
        if (!response.ok || payload.ok === false)
            throw new Error(payload.message || payload.error || "scan GitHub impossible");
...
function makeObsidianImportIndexes(entries) {
    ...
    for (const entry of entries) {
        const normalized = normalizeTitle(entry.baseTitle);
        if (!duplicateTitles.has(normalized))
            titleToTitle.set(normalized, entry.title);
    }
...
function resolveObsidianNoteTitle(rawTarget, currentTitle, indexes) {
    ...
    if (pathTarget && indexes.pathToTitle.has(pathTarget))
        return indexes.pathToTitle.get(pathTarget);
    const baseTarget = normalizeTitle(basenameWithoutMarkdown(notePath));
    if (baseTarget && indexes.titleToTitle.has(baseTarget))
        return indexes.titleToTitle.get(baseTarget);
    if (obsidianTargetHasAttachmentExtension(notePath))
        return "";
...
async function importObsidianVault(fileList) {
    ...
    try {
        ...
        if (!payload.notes.length)
            throw new Error("Aucune note Markdown trouvée");
...
function applyImportedVaultPayload(parsed, sourceLabel = "Import") {
    if (!requireVaultUnlocked("importer un contexte"))
        return;
    const payload = normalizeVaultImportPayload(parsed);
    if (!payload.notes.length)
        throw new Error("Aucune note valide");
    ...
    if (replace) {
        ...
        state.enrichmentRuns = Array.isArray(payload.enrichmentRuns)
            ? payload.enrichmentRuns.map(cleanEnrichmentRun).filter(Boolean).slice(0, MAX_ENRICHMENT_RUNS)
            : [];
        state.cognitiveCycles = Array.isArray(payload.cognitiveCycles)
            ? payload.cognitiveCycles.map(cleanCognitiveCycle).filter(Boolean).slice(0, MAX_COGNITIVE_CYCLES)
            : [];
        ...
        if (Array.isArray(payload.enrichmentRuns) && payload.enrichmentRuns.length) {
            const importedRuns = payload.enrichmentRuns.map(cleanEnrichmentRun).filter(Boolean);
        ...
        if (Array.isArray(payload.cognitiveCycles) && payload.cognitiveCycles.length) {
            const importedCycles = payload.cognitiveCycles.map(cleanCognitiveCycle).filter(Boolean);
...
function loadActiveVaultState() {
    ...
    state.enrichmentRuns = vaultEncrypted() ? [] : loadEnrichmentRuns();
    state.cognitiveCycles = vaultEncrypted() ? [] : loadCognitiveCycles();
    els.searchInput.value = "";
    loadStore();
...
async function switchVault(vaultId) {
    if (!vaultRegistry.vaults.some((vault) => vault.id === vaultId) || vaultId === activeVaultId)
        return;
    await persistActiveVaultBeforeLeaving();
...
function defaultTemplateTitle(template) {
    ...
    if (template.id === "project")
        return `Projet ${state.notes.filter((note) => normalizeFolder(note.folder) === "Projets").length + 1}`;
    if (template.id === "meeting")
        return `Réunion - ${date}`;
    if (template.id === "research")
        return `Recherche - ${date}`;
...
function createDailyNote() {
    if (!requireVaultUnlocked("ouvrir le journal"))
        return null;
...
function resetPanelLayout(kind = "all") {
    if (kind === "sidebar" || kind === "all")
        state.layout.sidebarWidth = DEFAULT_LAYOUT.sidebarWidth;
    if (kind === "inspector" || kind === "all")
        state.layout.inspectorWidth = DEFAULT_LAYOUT.inspectorWidth;
    if (kind === "editor" || kind === "all")
        state.layout.editorPaneWidth = DEFAULT_LAYOUT.editorPaneWidth;
    if (kind === "graph" || kind === "all")
        state.layout.graphHeight = DEFAULT_LAYOUT.graphHeight;
...
function startLayoutResize(kind, event) {
    if (window.matchMedia("(max-width: 1180px)").matches && kind !== "editor" && kind !== "graph")
        return;
...
function nudgePanelLayout(kind, direction) {
    ...
    if (kind === "sidebar")
        state.layout.sidebarWidth += step;
    if (kind === "inspector")
        state.layout.inspectorWidth -= step;
    if (kind === "editor")
        state.layout.editorPaneWidth = (state.layout.editorPaneWidth || currentEditorPaneWidth()) + step;
    if (kind === "graph")
        state.layout.graphHeight += step;
...
function resetDemo() {
    if (!requireVaultUnlocked("réinitialiser le vault"))
        return;
...
function bindEvents() {
    ...
    els.suggestionsList.addEventListener("click", (event) => {
        ...
        if (accept)
            void applyEnrichmentProposal(accept.dataset.proposalAccept);
        if (action)
            transformEnrichmentProposalToAction(action.dataset.proposalAction);
        if (reject)
            rejectEnrichmentProposal(reject.dataset.proposalReject);
    });
    els.wakeReviewPanel.addEventListener("click", (event) => {
        ...
        if (run)
            void runCognitiveWakeCycle();
        if (snapshot)
            void createVaultSnapshot();
    });
    ...
    els.folderFilters.addEventListener("click", (event) => {
        ...
        if (clear)
            state.folderFilter = null;
        if (folder)
            state.folderFilter = folder.dataset.folder;
        renderAll();
    });
    ...
    els.vaultDialog.addEventListener("click", (event) => {
        if (event.target === els.vaultDialog)
            closeVaultDialog();
    });
    ...
    document.addEventListener("keydown", (event) => {
        ...
        if (!els.commandPalette.hidden || !els.templatePicker.hidden || !els.vaultDialog.hidden)
            return;
...
// ----------------------------------------------------------------------
async function maybeImportServerVault() {
    let params;
    try {
    ...
    const known = vaultRegistry.vaults.find((v) => v.id === serverVaultId);
    if (known) {
        window.activeVaultId = serverVaultId;
        vaultRegistry.activeId = serverVaultId;
        persistVaultRegistry();
        loadActiveVaultState();
        renderAll();
        if (typeof toast === "function")
            toast(`Vault « ${known.name} » activé.`);
        return;
    }
    if (typeof toast === "function")
    ...
    try {
        ...
        const now = nowIso();
        const localNotes = serverNotes.map((n) => ({
            id: uid(),
        ...
        localStorage.setItem(vaultStorageKey("notes", serverVaultId), JSON.stringify(notesPayload, null, 2));
        loadActiveVaultState();
        renderAll();
        if (typeof toast === "function")
            toast(`Vault « ${name} » importé (${localNotes.length} note(s)).`);
    }
    catch (err) {
        console.error("[import-vault] failed:", err);
        if (typeof toast === "function")
            toast("Échec de l'import. Réseau ou auth ?");
    }
}
